...
class BudgetEntryRepository(
    ...
    override fun create(request: BudgetEntryRequest): BudgetEntry? {
        val newIid = dsl.insertInto(table)
            ...
            .set(table.CATEGORY_ID, request.categoryId)
            .set(table.VENDOR_ID, request.vendorId)
            .returning(idField)
            .fetchOne()
            ?.getValue(idField)
    ...
    override fun update(entity: BudgetEntry): BudgetEntry? {
        dsl.update(table)
            ...
            .set(table.CATEGORY_ID, entity.category?.id)
            .set(table.VENDOR_ID, entity.vendor?.id)
