    ...
    override fun map(record: Record?): BudgetEntry? {
        return record?.let {
            ...
            BudgetEntry(
                ...
                budgetCategory = if (categoryRecord.get(BUDGET_CATEGORY.ID) != null) categoryMapper.map(categoryRecord) else null,
                budgetVendor = if (vendorRecord.get(BUDGET_VENDOR.ID) != null) vendorMapper.map(vendorRecord) else null,
