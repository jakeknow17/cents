...
CREATE TABLE IF NOT EXISTS budget_category (
    ...
    CONSTRAINT unq_budget_category_uid          UNIQUE (uid),
    CONSTRAINT chk_budget_category_uid_len      CHECK (char_length(uid) = 21),
...
CREATE INDEX IF NOT EXISTS idx_budget_category_uid  ON budget_category (uid);
...
CREATE TABLE IF NOT EXISTS budget_vendor (
    ...
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT unq_budget_vendor_uid            UNIQUE (uid),
    CONSTRAINT chk_budget_vendor_uid_len        CHECK (char_length(uid) = 21),
    CONSTRAINT unq_budget_vendor_name           UNIQUE (name),
    CONSTRAINT chk_budget_vendor_name_len       CHECK (char_length(name) <= 255)
);
CREATE INDEX IF NOT EXISTS idx_budget_vendor_uid  ON budget_vendor (uid);
...
CREATE TABLE IF NOT EXISTS budget_entry (
    ...
    notes           TEXT NOT NULL,
    category_id     BIGINT REFERENCES budget_category (iid) ON DELETE SET NULL,
    vendor_id       BIGINT REFERENCES budget_vendor (iid) ON DELETE SET NULL,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT unq_budget_entry_uid             UNIQUE (uid),
    CONSTRAINT chk_budget_entry_uid_len         CHECK (char_length(uid) = 21),
    CONSTRAINT chk_budget_entry_notes_len       CHECK (char_length(notes) <= 1000)
);
CREATE INDEX IF NOT EXISTS idx_budget_entry_uid         ON budget_entry (uid);
