...
class BudgetEntryMapper(
    ...
    override fun map(record: Record?): BudgetEntry? {
        return record?.let {
            ...
            BudgetEntry(
                iid = it.getValue(BUDGET_ENTRY.IID, Long::class.java),
                uid = it.getValue(BUDGET_ENTRY.UID, String::class.java),
                entryDate = it.getValue(BUDGET_ENTRY.ENTRY_DATE, LocalDate::class.java),
                notes = it.getValue(BUDGET_ENTRY.NOTES, String::class.java),
                category = if (categoryRecord.get(BUDGET_CATEGORY.IID) != null) categoryMapper.map(categoryRecord) else null,
                vendor = if (vendorRecord.get(BUDGET_VENDOR.IID) != null) vendorMapper.map(vendorRecord) else null,
